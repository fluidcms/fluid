...
class MessageQueue
{
    ...
    public static function getAvalaiblePort($port = null, $loop = 0)
    {
        ...
        file_put_contents(Fluid::getConfig('storage') . self::$portFile, json_encode(array("port" => $port)));
        return $port;
    }
    ...
    public static function getPort()
    {
        ...
        if (is_file(Fluid::getConfig('storage') . self::$portFile)) {
            $content = file_get_contents(Fluid::getConfig('storage') . self::$portFile);
